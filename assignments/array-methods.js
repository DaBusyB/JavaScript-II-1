...
runners.forEach(function(eaRunner) {
    fullName.push(`${eaRunner.first_name} ${eaRunner.last_name}`);
})
...
runners.map(function(eaRunner) {
    allCaps.push(`${eaRunner.first_name}`.toUpperCase());
})
...
const lRunner = runners.filter( runner => runner.shirt_size === "L");
largeShirts.push(lRunner);
